...
use needles_in_haystack::{
    rabin_karp::RabinKarpHaystack, suffix_array::SuffixArrayHaystack,
    suffix_tree::SuffixTreeHaystack, window::WindowHaystack, Haystack,
};
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("suffix-array", |b| {
        let h = SuffixArrayHaystack::new(haystack());
        let n = needle();
        b.iter(|| h.contains(&n))
    });
}
